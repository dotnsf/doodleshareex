...
//. Redis（サーバーと接続する）
var redis = settings_redis_url ? ( new Redis( settings_redis_url ) ) : ( new Redis( settings_redis_port, settings_redis_server ) );
...
app.get( '/client', function( req, res ){
  ...
  res.render( 'client1', { name: name, room: room } );
});
...
function subscribeMessage( room ){
  if( rooms.indexOf( room ) == -1 ){
    ...
    console.log( 'client', room, clients[room] );
    ...
    clients[room].on( 'message', function( room, message ){
      ...
      console.log( 'client.message: broadcast', room, message );  //. このサーバーに接続している全ウェブソケットクライアントにブロードキャスト
...
function broadcast( room, message ){
  wss.clients.forEach( function( client ){
    console.log( 'broadcast', JSON.stringify( client ), message );     //. 個別の client を識別する id は？？
